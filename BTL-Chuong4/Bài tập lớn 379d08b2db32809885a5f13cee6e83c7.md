...
| --- | --- | --- | --- |
| pfSense | Route + Firewall
Server VLAN
DMZ VLAN | 192.168.92.138
192.168.100.1
192.168.200.1 | 00:0C:29:E6:4F:00
00:0C:29:E6:4F:0A
00:0C:29:E6:4F:14 |
...
**Lưu ý :** Tắt tính năng **DHCP Server** nội bộ của pfSense trên cổng LAN (.100.1), vào mục *Services > DHCP Server > tab LAN >* **Disable** nó đi. pfSense không cho phép bật song song cả DHCP Server và DHCP Relay trên cùng hệ thống và sẽ xung đột DHCP service với DC-01
...
`587`: Submission
`993`: IMAPS
`8081` : Web Haproxy | Nhận traffic HTTPS và các luồng mail từ HAProxy chuyển tiếp xuống. |
|  |  | Nhận traffic Web từ cổng 80 của HAProxy chuyển tiếp xuống cổng 8081. |
...
#trên máy client (máy không phải ansible)
sudo visudo
...
[Cách webserver chạy 2 máy thì dữ liệu đồng bộ như thế nào (High Availability cho web).](https://app.notion.com/p/C-ch-webserver-ch-y-2-m-y-th-d-li-u-ng-b-nh-th-n-o-High-Availability-cho-web-381d08b2db3280a7b8eef0bab461bc8f?pvs=21)

[Cách mailserver chạy 2 máy thì dữ liệu đồng bộ như thế nào (High Availability cho mail).](https://app.notion.com/p/C-ch-mailserver-ch-y-2-m-y-th-d-li-u-ng-b-nh-th-n-o-High-Availability-cho-mail-381d08b2db3280a493b0f6456490ee41?pvs=21)
...
2. Chạy Playbook
    ...
    ```bash
    #Lỗi yêu cầu pass sudo khi chạy ansible
    #Trong bài này HAProxy cài trên máy sử dụng Ansible nên cần thêm cả 3 máy
    sudo visudo
